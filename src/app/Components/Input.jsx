...
function Input({name, id, title, className, type, value, onchange, labelClass}) {
    return (
        <div className={"inputField relative mt-6" + ` ${className}`}>
            <input type={type} name={name} id={id} className={'bg-transparent w-full py-2 border-b border-gray-400 peer outline-none text-base font-light'} required value={value} onChange={onchange}/>
